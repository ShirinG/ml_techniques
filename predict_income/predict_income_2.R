...
bst.cv <- xgb.cv(params=paramList, data=trainM, label=as.matrix(train$Income), nfold=5, early_stopping_rounds=10, eval_metric="rmse", nrounds=200, prediction=TRUE)
bst <- xgboost(params=paramList, data=trainM, label=as.matrix(train$Income), nrounds=nrow(bst.cv[["evaluation_log"]])-10)
