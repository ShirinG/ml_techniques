...
train <- fread("predict_income/train.csv")
test <- fread("predict_income/test.csv")
...
bst.cv <- xgb.cv(params=paramList, data=as.matrix(train_new[, features, with=FALSE]), label=as.matrix(train_new$Income), folds=testIdxs, early_stopping_rounds=3, eval_metric="rmse", nrounds=200, prediction=TRUE)
bst <- xgboost(params=paramList, data=as.matrix(train_new[, features, with=FALSE]), label=as.matrix(train_new$Income), nrounds=nrow(bst.cv[["evaluation_log"]])-3)
